...
Ext.extend(Xietong.manage.TaskManage,Ext.util.Observable,{
	init:function(){
			...
			handler:function() {
				if(searchPeronComboBox.isValid()) {
					ds.proxy = new Ext.data.HttpProxy({
					   	url:'module/task/listSchedule.do'
					   });
		...
		var mainPanel = new Ext.grid.EditorGridPanel({
			renderTo:'task-schedule-main',
			height:Ext.get("task-schedule-main").getHeight(),
			width:Ext.get("task-schedule-main").getWidth(),
			cm:cm,
			sm:sm,
			store:ds,
			plugins: expander,
		...
		var borderPanel = new Ext.Panel({
			...
			items:[{
				...
				layout: 'fit',
				border: false,
				height: Ext.get("task-schedule-main").getHeight()*0.2,
				items: [logPanel]
			}]
		});
	},
